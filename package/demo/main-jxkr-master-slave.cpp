...
int main(int argc, char *argv[])
{
    ...
    std::thread t([mode]
        ...
        // 待写入的文件
        FILE *ptr;
        std::string f = "./cache/" + std::string(mode == QM_MASTER ? "master" : "slave") + "_data.csv";
        ptr = fopen(f.c_str(), "wb");
        const char* title = "UpdateTime,UpdateMillisec,InstrumentId,LastPrice,Volume,Turnover,OpenInterest,UpperLimitPrice,LowerLimitPrice,AveragePrice,BidPrices,BidVolumes,AskPrices,AskVolumes\n";
        ...
        sprintf(data, "%d,%03d,%s,%.2f,%d,%.2f,%d,%lu,%llu,%lu,\"%.2f,%.2f,%.2f,%.2f,%.2f\",\"%d,%d,%d,%d,%d\",\"%.2f,%.2f,%.2f,%.2f,%.2f\",\"%d,%d,%d,%d,%d\"\n", 
                ret->snap.snapTime,
