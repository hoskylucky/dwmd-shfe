...
void check_dir_alive(char *path){
	DIR *dir;
	dir = opendir(path);
	if(NULL==dir){
		mkdir(path,0777);
...
void log_info(const char *fmt,...){
	...
	if(access(filename,F_OK) == 0){
		...
		if(file_stat.st_size > LOG_FILE_MAX_SIZE){
			truncate(filename,0);
		}
	}
...
#endif
static void write_le32(volatile unsigned char  *dev,unsigned int  addr,unsigned int data){
...
static int align_pagesize(unsigned int len){
	int n = getpagesize();
	return ((len + n -1 ) & (~(n - 1)));
}

int reg_write(unsigned char *addr,unsigned int offset,unsigned int val){
		unsigned int retval = 0;
		write_le32(addr,offset,val);
		retval = read_le32(addr,offset);
		if(retval != val)
			return ERROR_CONF_WRITE_FAIL;
		return RET_SUCESS;		 
}


void clear_bar_map(unsigned char *bar_addr,unsigned int addr,unsigned int int_count){
	unsigned int i=0;
...
int32_t config_udp(unsigned char *bar_addr,cfg_mirp* pMirp){
	...
	for(i=0;i<pMirp->cfg_num;i++){
		if(pMirp->cfgs[i].porteEn_0){
			...
			log_info("A UDP IP:0x%x,No:%d,Post=%d,ADDR=0x%x",pMirp->cfgs[i].addr,anum,apost,MIRP_ABASE_ADDR + offset);
			ret = reg_write(bar_addr,MIRP_ABASE_ADDR + offset + 4,pMirp->cfgs[i].port);
			if(ret != RET_SUCESS){
				err_num=ret;
				goto err;
			}
			log_info("A UDP Port:%d,No:%d,Post=%d,ADDR=0x%x",pMirp->cfgs[i].port,anum,apost,MIRP_ABASE_ADDR + offset + 4);
		...
		if(pMirp->cfgs[i].porteEn_1){
			...
			log_info("B UDP IP:0x%x,No:%d,Post=%d,ADDR=0x%x",pMirp->cfgs[i].addr,bnum,bpost,MIRP_BBASE_ADDR + offset);
			ret = reg_write(bar_addr,MIRP_BBASE_ADDR + offset + 4,pMirp->cfgs[i].port);
			if(ret != RET_SUCESS){
				err_num=ret;
				goto err;
			}
			log_info("B UDP Port:%d,No:%d,Post=%d,ADDR=0x%x",pMirp->cfgs[i].port,bnum,bpost,MIRP_BBASE_ADDR + offset + 4);
...
void read_fpga_state(int32_t *state, monitor_union* monitor, char *msg, int32_t msgSize){
	int err_num=RET_SUCESS;
		...
		monitor->IP_AUDP_INE[0]=*(volatile unsigned int *)(bar0_addr + MIRP_ABASE_ADDR + TOPIC_5000_UDP_POST * 16);
		monitor->IP_AUDP_INE[1]=*(volatile unsigned int *)(bar0_addr + MIRP_ABASE_ADDR + TOPIC_5000_UDP_POST * 16 + 8);
		monitor->Port_A_INE[0] =*(volatile unsigned int *)(bar0_addr + MIRP_ABASE_ADDR + TOPIC_5000_UDP_POST * 16 + 4);
		monitor->Port_A_INE[1] =*(volatile unsigned int *)(bar0_addr + MIRP_ABASE_ADDR + TOPIC_5000_UDP_POST * 16 + 12);

		monitor->IP_AUDP_METAL[0] =*(volatile unsigned int *)(bar0_addr + MIRP_ABASE_ADDR + TOPIC_2000_UDP_POST * 16);
		monitor->IP_AUDP_METAL[1] =*(volatile unsigned int *)(bar0_addr + MIRP_ABASE_ADDR + TOPIC_2000_UDP_POST * 16 + 8);
		monitor->Port_A_METAL[0]  =*(volatile unsigned int *)(bar0_addr + MIRP_ABASE_ADDR + TOPIC_2000_UDP_POST * 16 + 4);
		monitor->Port_A_METAL[1]  =*(volatile unsigned int *)(bar0_addr + MIRP_ABASE_ADDR + TOPIC_2000_UDP_POST * 16 + 12);
		...
		monitor->IP_BUDP_INE[0] =*(volatile unsigned int *)(bar0_addr + MIRP_BBASE_ADDR + TOPIC_5000_UDP_POST * 16);
		monitor->IP_BUDP_INE[1] =*(volatile unsigned int *)(bar0_addr + MIRP_BBASE_ADDR + TOPIC_5000_UDP_POST * 16 + 8);
		monitor->Port_B_INE[0]  =*(volatile unsigned int *)(bar0_addr + MIRP_BBASE_ADDR + TOPIC_5000_UDP_POST * 16 + 4);
		monitor->Port_B_INE[1]  =*(volatile unsigned int *)(bar0_addr + MIRP_BBASE_ADDR + TOPIC_5000_UDP_POST * 16 + 12);

		monitor->IP_BUDP_METAL[0] =*(volatile unsigned int *)(bar0_addr + MIRP_BBASE_ADDR + TOPIC_2000_UDP_POST * 16);
		monitor->IP_BUDP_METAL[1] =*(volatile unsigned int *)(bar0_addr + MIRP_BBASE_ADDR + TOPIC_2000_UDP_POST * 16 + 8);
		monitor->Port_B_METAL[0]  =*(volatile unsigned int *)(bar0_addr + MIRP_BBASE_ADDR + TOPIC_2000_UDP_POST * 16 + 4);
		monitor->Port_B_METAL[1]  =*(volatile unsigned int *)(bar0_addr + MIRP_BBASE_ADDR + TOPIC_2000_UDP_POST * 16 + 12);
...
#define NET_STU_MASK 0x1F
		ExceptionEvent_value |= (*(volatile unsigned int *)(bar0_addr + NET_LINK_STU_REG))&NET_STU_MASK;
...
#define CENTER_CD_MASK           0xffff0000
		...
		if(value){
			if(value & CENTER_AB_MASK)
				center_change_flag |= 0x1;
			if(value & CENTER_CD_MASK)
				center_change_flag |= 0x2;
		...
		if(center_change_flag){
			ExceptionEvent_value |= center_change_flag << 9;
		}
...
int init_fpga(void * param){
	...
	if(*(unsigned int *)(bar0_addr + PCIE_DMA_DST_ADDR_H) != (c2h_phy_addr >> 32)){
		...
		log_info("PCIE_DMA_DST_ADDR_H ADDR0x=%x--VALUE0x:%x",PCIE_DMA_DST_ADDR_H,c2h_phy_addr >> 32);
	...
	if((err_num=config_udp(bar0_addr,&conf.Mirp))!= RET_SUCESS){
		goto err;
	}
